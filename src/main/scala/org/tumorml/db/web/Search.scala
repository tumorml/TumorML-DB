...
import java.io._
import org.tumorml.db.{Database, WebStack}
...
class Search extends WebStack {
  ...
  get ("/") {
    if (params.get("q")==None)
      jade("/index", "title" -> "TumorML Database Search")
    else {
      val queryDocsByTitle = "db:open('tumorml')/descendant::tumorml[header/title contains text '" + params({"q"}) +"']"
      ...
      jade("/results", "title" -> "results", "query" -> params("q"), "results" -> resultsList.toList)
    }
  }
